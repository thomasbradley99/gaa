...
SCHEMA_FILE = SCHEMA_DIR / "schema_41_detectable_events.json"
...
def generate_timeline(matches, overall_metrics, total_ai, total_pro, output_dir, event_stats):
    ...
    # Custom event order by importance (goals first, then paired home/opp for each type)
    event_order = [
        ...
        "Home Goal Kick", "Opp Goal Kick",
        "Home Throw In", "Opp Throw In",
        "Home Kick Off", "Opp Kick Off",
        # Discipline
        "Home Foul", "Opp Foul",
        ...
        # Match structure
        "1st Half Start", "2nd Half Start", "1st Half End", "2nd Half End",
    ]
    
    # Build sorted list: ordered events first, then any remaining
    sorted_events = []
    for event_type in event_order:
        if event_type in event_stats:
            sorted_events.append((event_type, event_stats[event_type]))
    
    # Add any events not in our order (shouldn't happen, but just in case)
    for event_type, stats in event_stats.items():
        if event_type not in event_order:
            sorted_events.append((event_type, stats))
