/**
 * Export clubs NOT using VEO to CSV file
 */
...
const OUTPUT_FILE = path.join(__dirname, '../crm-data/veo/clubs_not_using_veo.csv');

async function exportClubsNotUsingVeo() {
  try {
    console.log('📊 Querying clubs NOT using VEO from database...');
    ...
    const result = await pool.query(
      `SELECT 
        ...
        veo_club_identifier
      FROM clubs 
      WHERE uses_veo = false 
      ORDER BY club_name ASC`
    );
    
    console.log(`✅ Found ${result.rows.length} clubs NOT using VEO`);
