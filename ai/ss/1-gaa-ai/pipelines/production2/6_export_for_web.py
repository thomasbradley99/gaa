...
def parse_observations(obs_file: Path):
    """Parse observations.txt to web format - handles both [XXXs] and [XXXs-YYYs] formats"""
    ...
    for block in content.split('\n['):
        ...
        # Extract timestamp from [XXXs] or [XXXs-YYYs] format (use start time)
        if block.startswith('['):
            # Try [XXXs-YYYs] format first (40s clips with overlap)
            if 's-' in block and 's]' in block:
                timestamp_str = block.split('s-')[0].replace('[', '')
                try:
                    timestamp = int(timestamp_str)
                    # Get description (everything after the timestamp range)
                    ...
                    continue
            # Try [XXXs] format (original 60s clips)
            elif 's]' in block:
                timestamp_str = block.split('s]')[0].replace('[', '')
                try:
                    timestamp = int(timestamp_str)
...
def main():
    ...
    # Use full game XML if available (for complete timeline), otherwise fall back to filtered
    pro_xml = pro_xml_full if pro_xml_full.exists() else pro_xml_filtered
    ...
    print(f"\n📤 Upload to S3:")
    print(f"   aws s3 cp {BASE_DIR}/*.js s3://end-nov-webapp-clann/football-analysis/{args.game}/")
    print(f"\n🌐 Website URL (after upload):")
    print(f"   https://end-nov-webapp-clann.s3.eu-west-1.amazonaws.com/football-analysis/game_viewer.html?game={args.game}&audio=with-audio")
