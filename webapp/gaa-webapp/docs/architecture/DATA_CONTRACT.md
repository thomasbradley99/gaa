...
**Status:** ✅ PRODUCTION (reflects current implementation after Nov 19 fixes)  
**Purpose:** Define the exact data structures used across Lambda → Backend → Frontend
...
```typescript
{
  ...
  // ===== METADATA (JSONB) - 🔴 CRITICAL FOR DISPLAY =====
  metadata?: {
    ...
    // Team color mapping (AI-detected → home/away)
    team_mapping?: {
...
```typescript
// Expects:
game.video_url              // ✅ Required - video source
game.hls_url                // ⚠️ Optional - HLS if available
game.title                  // ✅ Required - display name

// Team colors (CRITICAL):
game.metadata.teams.home_team.name          // ❌ Currently missing
game.metadata.teams.home_team.jersey_color  // ❌ Currently missing
game.metadata.teams.away_team.name          // ❌ Currently missing
game.metadata.teams.away_team.jersey_color  // ❌ Currently missing
...
dbEvents.map(e => {
  ...
  return {
    ...
    timestamp: e.time,             // Map 'time' → 'timestamp'
...
- Home events → `game.metadata.teams.home_team.jersey_color`
- Away events → `game.metadata.teams.away_team.jersey_color`

Without this data:
- ❌ Timeline shows all events as black/white
- ❌ No visual distinction between teams
- ❌ Looks boring/confusing
...
## 🚀 Lambda Output (CURRENT IMPLEMENTATION)
...
UPDATE games
SET 
  events = $1::jsonb,      // ← Direct array of events
  metadata = $2::jsonb,    // ← team_mapping, match_info, teams
  status = 'analyzed'
WHERE id = game_id
...
## 🔴 Current Gaps (What Still Needs Fixing)

### 1. ✅ FIXED: Events Structure
...
- **Status**: ✅ Working - Stats show correct scores

### 3. ✅ FIXED: Event Type Mapping
- **Was**: All events mapped to `type: 'shot'`
- **Now**: Maps `action + outcome` → specific type (`point`, `wide`, `goal`)
- **Status**: ✅ Working - Event filters work correctly

### 4. ⚠️ PARTIAL: Team Colors
...
- **Why**: User's team colors (from DB) not passed from Lambda 1 → Lambda 2
- **Impact**: Timeline shows correct detected colors, but not user's club colors

### 5. ❌ MISSING: Opposition Info
...
1. ✅ Deploy updated Lambda 2
2. ✅ Submit a test VEO URL
...
## 🎯 Summary (Current State - Nov 19, 2025)

### ✅ What's Working

**Database Structure:**
- Events stored as **direct JSONB array** ✅
- Metadata stored separately with team info, team_mapping, match_info ✅
- Backend correctly extracts events from Lambda payload ✅

**Frontend Display:**
- Events tab shows all events correctly ✅
- Event type filters work (Point, Wide, Goal, Kickout, etc.) ✅
- Stats tab calculates scores correctly (HOME 0-04, AWAY 0-02) ✅
- Event transformer maps `action + outcome → type` ✅

**Lambda Pipeline:**
- Detects team colors from video ✅
- Generates events with correct format ✅
- Posts to backend with metadata ✅
- Stores XML to S3 (for Anadi export) ✅
...
Backend creates game (status: "pending")
  ↓
Lambda 1 downloads video from VEO → S3
  ↓
...
Frontend fetches game
  ↓
Frontend transforms events (action → type mapping)
  ↓
Displays in Stats/Events/Coach tabs ✅
```
