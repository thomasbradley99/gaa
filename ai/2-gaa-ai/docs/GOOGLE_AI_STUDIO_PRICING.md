...
**Data Retrieved:** November 19, 2025
...
- **Input (prompts ≤ 200k tokens):** $2.00
- **Input (prompts > 200k tokens):** $4.00
- **Output (prompts ≤ 200k tokens):** $12.00
- **Output (prompts > 200k tokens):** $18.00
- **Context caching (≤ 200k):** $0.20
- **Context caching (> 200k):** $0.40
...
### Grounding
- **Google Search:** 1,500 RPD (free), then $14 / 1,000 search queries (coming soon)
- **Google Maps:** Not available

### Notes
- High-performance multimodal model
- Advanced reasoning capabilities
- Tiered pricing based on prompt size (200k threshold)
...
def calculate_cost(input_tokens, output_tokens, model="gemini-2.0-flash", 
    ...
    pricing = {
        ...
        "gemini-3.0": {
            "input_small": 2.00 / 1_000_000,  # <= 200k tokens
            "input_large": 4.00 / 1_000_000,  # > 200k tokens
            "output_small": 12.00 / 1_000_000,
            "output_large": 18.00 / 1_000_000
        },
    ...
    if model in ["gemini-2.5-pro", "gemini-3.0", "gemini-3-pro-preview"]:
        # Pro and Gemini 3 have different pricing for large prompts
...
| Gemini 3 | $0.2450 | $0.1140 | **$0.3590** |

**Recommendation:** Use **gemini-2.0-flash** - best balance of cost and quality.

**Note:** Gemini 3 is the most expensive option and has no free tier, making it 22x more costly than 2.0 Flash for our pipeline.
