...
3. Game state - choose ONE:
   - "KICKOFF" - ball at center circle, players lined up for kickoff
...
Format example: "Blue vs White. Pink LEFT, Green RIGHT. KICKOFF - ball at center, teams lined up. Active."
...
def calibrate_game():
    ...
    if not frames:
        ...
        print(f"Run: python 0_generate_clips_and_frames.py --game {ARGS.game}")
        return False
    ...
    synthesis_prompt = f"""Based on these frame descriptions from a football match, create a game profile.
...
2. **MATCH TIMES:**
   - Match START: Find the FIRST timestamp with "KICKOFF" state
   - Half Time: Find first timestamp with "HALFTIME" state (players walking off)
   - 2nd Half START: Find the SECOND "KICKOFF" state (after halftime)
